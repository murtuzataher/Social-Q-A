...
<div class="main style2">
  ...
  <div class="content">
    <div class="content_resize">
      <div class="mainbar">
	  <h2 class="style7">Answer Status</h2>
	   <%
	  try{	    
        ...
        ResultSet rs=st.executeQuery(query);
			if ( rs.next()==true )
				...
				dcount=0;
				String query2="select * from document "; 
        		Statement st2=connection.createStatement();
       			ResultSet rs2=st2.executeQuery(query2);
					while ( rs2.next()==true )//1 open
	   					{	
							int i1=rs2.getInt(1);
							...
							String Con=rs2.getString(7);
									   ...
									   String decryptedDes= new String(Base64.decode(Con.getBytes())).toLowerCase();
							
								if(decryptedDes.contains(q2))
								{
								
								if(ansname.equals(uname)){
								dcount++;
								 %>
							 ...
							 <h3 class="style7"><span class="style6">Answer For Your Question Is=</span><%=decryptedDes%> </h3>
							 <%
							}
							...
							String sql8="select * from req_res where ((requestfrom='"+uname+"'&&requestto='"+ansname+"') || (requestfrom='"+ansname+"'&&requestto='"+uname+"')) && status='Accepted'";
								...
								String sql11="select * from interests where username='"+ansname+"' and category='"+s10+"' ";
							...
							String query320 ="update samequestions set answered='"+an1+"' where question LIKE '%"+q21+"%' and qusername='"+uname+"' ";
									...
									if(ask1.equalsIgnoreCase("ask")){
									  		...
									  		String query123="select rank from document where dname='"+daname+"' "; 
									  	...
									  	String query321 ="update document set rank='"+rank123+"' where dname='"+daname+"' ";
...
PreparedStatement ps=connection.prepareStatement("insert into autoanswers(qusername,question,ausername,dname,answer,adate,did) values (?,?,?,?,?,?,?)"); 
						...
						ps.executeUpdate();
								...
								%>
								 ...
								 <img align="middle" height="100" width="120" src="images.jsp?value=<%="image"%>&id=<%=i1%>"/>
								<h3><span class="style8">Answer For This Question Given By=</span><%=ansname%> </h3>
	   ...
	   <%
					...
					{
					dcount1=0;
				String query3="select * from answers "; 
					...
					while ( rs3.next()==true )
	  					{
							...
							answerby=rs3.getString(7);
										...
										Cipher c2 = Cipher.getInstance("AES");
										c2.init(Cipher.ENCRYPT_MODE, key);
							String decryptedDes= new String(Base64.decode(answer.getBytes())).toLowerCase();
							
					if(decryptedDes.contains(q2))
						{
							if(answerby.equals(uname)){
								dcount1++;
								 %>
							...
							String sql81="select * from req_res where ((requestfrom='"+uname+"'&&requestto='"+answerby+"') || (requestfrom='"+answerby+"'&&requestto='"+uname+"')) && status='Accepted'";
								...
								String sql11="select * from interests where username='"+answerby+"' and category='"+s10+"' ";
								Statement st11=connection.createStatement();
								ResultSet rs11=st11.executeQuery(sql11);
					   			if( rs11.next() )
								{
...
PreparedStatement ps=connection.prepareStatement("insert into frdsanswers(qusername,question,ausername,answer,adate,aid) values (?,?,?,?,?,?)"); 
						ps.setString(1,uname);
						ps.setString(2,q31);
						ps.setString(3,answerby);
						...
						ps.executeUpdate();
								...
								%>
								<img align="middle" height="100" width="120" src="images.jsp?value=<%="answer"%>&id=<%=i2%>"/>
								<h3><span class="style8">Answer For This Question Given By=</span><%=answerby%> </h3>
								<h3><span class="style6">Answer For Your Question Is=</span> <%=decryptedDes%> </h3>
	   <%
				}}}}
				
				
					if(dcount==0 && dcount1==0){
					...
					while ( rs2.next()==true )//1 open
	   					{	
							...
							String Con=rs2.getString(7);
							 ...
							 <img align="middle" height="100" width="120" src="images.jsp?value=<%="image"%>&id=<%=i1%>"/>
							 <h3><span class="style8">Answer For This Question Given By=</span> <%="you"%> </h3>
							 <h3 class="style7"><span class="style8">Answer For This Question Is=</span> <%=decryptedDes%> </h3>
							 <%
							}
